    ...
    try {
        ...
        if (data.movies && data.movies.length > 0) {
            data.movies.forEach(movie => {
                // Ensure the movie object has 'id' if the backend provides it, otherwise mapped in App.jsx
          ...
          {messages.map((msg, index) => (
            <div key={index} style={{ display: 'flex', justifyContent: msg.type === 'user' ? 'flex-end' : 'flex-start', marginBottom: '20px' }}>
              {msg.isJson ? (
                <MovieCard 
                  data={msg.content} 
                  // Check if title exists in watchlist to show red heart
